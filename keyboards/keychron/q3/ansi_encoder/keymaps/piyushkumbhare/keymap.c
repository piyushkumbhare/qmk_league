...
void raw_hid_receive(uint8_t* data, uint8_t length) {
    ...
    if(data[0] > RGB_MATRIX_LED_COUNT) {
        for(int i = 0; i < RGB_MATRIX_LED_COUNT; i++) {
            led_colors[i].ignore = 1;
        }
    } else {
        led_colors[data[0]].ignore = 0;
        led_colors[data[0]].color.r = 255;
    }

}

bool rgb_matrix_indicators_advanced_user(uint8_t led_min, uint8_t led_max) {
    for(int i = led_min; i < led_max && !led_colors[i].ignore; i++) {
